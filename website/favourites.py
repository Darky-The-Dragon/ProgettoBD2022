...
def follow_artist(id_artist):
    ...
    if s:
        flash('You are already following this artist', category='error')
        ...
        db.session.commit()
        flash('You are now following this artist', category='success')
...
@login_required
def unfollow_artist(id_artist):
    listener_artist = select([favourites_artist]).where(favourites_artist.c.id_artist == id_artist,
                                                        favourites_artist.c.id_listener == current_user.id)
    s = db.session.execute(listener_artist).first()
    if s is None:
        flash('You can\'t unfollow this artist. You are not following him yet', category='error')
    else:
        ...
        db.session.commit()
        flash('You unfollowed this artist', category='success')
